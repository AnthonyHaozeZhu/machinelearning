...
STATE_DICT_FILE = '0112epoch2000.pkl'
autoencoder = AutoEncoder(1000,hasBN=False)
...
if __name__ == '__main__':
    ...
    autoencoder.load_state_dict(torch.load(STATE_DICT_FILE))
    print(autoencoder)
    PI, M, THETA = autoencoder(testdata)
    iszero = testdata == 0
    predict_dropout_of_all = PI>0.5


    true_drop_out_mask = iszero*((testtruedata - testdata)!=0)
    predict_dropout_mask = iszero*predict_dropout_of_all
    after = torch.floor(torch.where(predict_dropout_mask,M,testdata))
    zero_num = iszero.sum()
    true_dropout_num = true_drop_out_mask.sum()
    predict_dropout_num = predict_dropout_mask.sum()
    predict_correct = (predict_dropout_mask*true_drop_out_mask).sum()
    ...
    dif_after =  testtruedata - after
    dif_true = testtruedata - testdata
    # print(dif_after)
    # print(dif_true)
    print("predict distance:", torch.sqrt(torch.square(testtruedata - after).sum()).data,
          "origin distance:", torch.sqrt(torch.square(testtruedata - testdata).sum()).data)
